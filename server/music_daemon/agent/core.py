...
class AnimusPersonalAgent:
    ...
    def _handle_executable_intent(
        ...
        if intent.primary_intent == "SCHEDULE_ACTION":
            ...
            if act_type == "SET_AC_TEMPERATURE":
                ...
                msg = f"Okay {addr}. I'll set the AC to {temp}°C in {mins} minutes."
            elif act_type == "MEDIA_STOP":
                msg = f"Okay {addr}. I'll stop the movie in {mins} minutes."
            elif act_type == "PROJECTOR_POWER_OFF":
                msg = f"Okay {addr}. I'll turn off the projector in {mins} minutes."
            else:
                msg = f"Okay {addr}. Scheduled for {mins} minutes from now."
        ...
        if intent.primary_intent in ("MEDIA_PAUSE", "PAUSE_MEDIA"):
            ...
            plan = GeminiStructuredPlan(
                ...
                user_request=utterance,
                steps=[PlanStep(step_id=1, device="FIRE_TV", capability="FIRE_TV_MEDIA_PAUSE")]
            )
            exec_success = True
            if self.planner_executor:
                val_res = self.planner_executor.validator.validate_plan(plan_input=plan, room_state=current_state)
            ...
            res = AgentInteractionResult(
                ...
                target_device="FIRE_TV",
                human_response=msg,
                execution_attempted=True,
                execution_success=exec_success,
                state_delta=StateDelta(subsystem="MEDIA", attribute="media_playback", previous_value="PLAYING", new_value="PAUSED", verified_value="PAUSED")
            )
            self.context_buffer.record_interaction_result(res)
            return AgentInteractionResponse(
                understood_intent=intent.primary_intent,
                ...
                physical_audits=[
                    PhysicalActionAuditRecord(
                        intent="PAUSE_MEDIA",
                        target="FIRE_TV",
                        capability="FIRE_TV_MEDIA_PAUSE",
        ...
        if intent.primary_intent in ("MEDIA_RESUME", "RESUME_MEDIA"):
            ...
            plan = GeminiStructuredPlan(
                ...
                user_request=utterance,
                steps=[PlanStep(step_id=1, device="FIRE_TV", capability="FIRE_TV_MEDIA_PLAY")]
            )
            exec_success = True
            if self.planner_executor:
                val_res = self.planner_executor.validator.validate_plan(plan_input=plan, room_state=current_state)
            ...
            res = AgentInteractionResult(
                ...
                execution_attempted=True,
                execution_success=exec_success,
                state_delta=StateDelta(subsystem="MEDIA", attribute="media_playback", previous_value="PAUSED", new_value="PLAYING", verified_value="PLAYING")
            )
            self.context_buffer.record_interaction_result(res)
            return AgentInteractionResponse(
                understood_intent=intent.primary_intent,
            ...
            try:
                ...
                if val_res:
                    ...
                    for s_res in exec_res.steps:
                        ...
                        step_verif_map[cap_id] = (s_res.status == ExecutionStatus.VERIFIED)
                    ...
                    if intent.primary_intent == "SET_AC_TEMPERATURE" or (physical_audits and physical_audits[0].capability == "AC_SET_TEMPERATURE"):
                        ...
                        if ac_step_verified and exec_res.success:
                            ...
                            verified_t = ac_readback.get("temperature") or ac_readback.get("target_temperature")
                            if verified_t is None and current_state and hasattr(current_state, "ac") and current_state.ac:
                                verified_t = current_state.ac.target_temperature.value
                        ...
                        if step_verif_map.get(cap_name, exec_res.success) and exec_res.success:
                            ac_readback = step_res_map.get(cap_name, {})
                            if "power" in ac_readback:
                                verified_pow = bool(ac_readback["power"])
                            elif current_state and hasattr(current_state, "ac") and current_state.ac:
                                verified_pow = current_state.ac.power.value
                    ...
                    elif intent.primary_intent in ("PROJECTOR_POWER_WAKE", "PROJECTOR_POWER_SLEEP") or (physical_audits and physical_audits[0].capability in ("PROJECTOR_POWER_WAKE", "PROJECTOR_POWER_SLEEP")):
                        ...
                        verified_proj = None
                        if step_verif_map.get(cap_name, exec_res.success) and exec_res.success:
                            proj_readback = step_res_map.get(cap_name, {})
                            if "power" in proj_readback:
                                verified_proj = bool(proj_readback["power"])
                            elif current_state and hasattr(current_state, "projector") and current_state.projector:
                                verified_proj = current_state.projector.power.value
                    ...
                    elif "LAUNCH_" in intent.primary_intent or "PLAY_" in intent.primary_intent:
    ...
    def _synthesize_canonical_plan(self, utterance: str, room_state: Any) -> Optional[GeminiStructuredPlan]:
        ...
        from agent.intent_resolver import WORD_TO_NUM
        ...
        if any(w in lower for w in ["turn off the projector", "turn off projector", "power off projector", "sleep projector", "projector off", "switch off the projector", "switch off projector"]):
        ...
        # 3. Cinema / Movie ("Let's watch something", "movie mode", "chill with a movie")
        ...
        # 4. In-flight YouTube / Provider Correction Switch
        if "youtube" in lower and "actually" in lower:
        ...
        if any(r_w in lower for r_w in ["resume it", "resume that", "resume this", "resume video", "resume media", "resume playback", "unpause", "continue watching", "resume"]):
        ...
        # 7. Volume Down / "Make it quieter" / "Turn the room down" / "Turn it down a bit"
        if any(w in lower for w in ["quieter", "turn down", "turn it down", "turn it down a bit", "softer", "turn the room down", "make it quiet", "that's too loud", "lower the volume", "lower the volume a little"]):
            active_prod = "FIRE_TV"
        ...
        # 8. Volume Up / "Make it louder" / "Turn it up a bit"
        if any(w in lower for w in ["louder", "turn up", "turn it up", "turn it up a bit", "volume up", "increase the volume", "raise the volume"]):
            active_prod = "FIRE_TV"
            if room_state and hasattr(room_state, "audio_stream") and room_state.audio_stream:
                prod = getattr(room_state.audio_stream, "active_producer", None)
                if prod:
        ...
        # 13. AC Temperature (Turns 07, 25, 43: "Set the AC to 24", "Set AC to 25", "AC 24", "make it 24 degrees")
        ac_m = re.search(r'(?:(?:ac|temperature|temp)\s+(?:to\s+)?(\d+)|(?:set\s+(?:the\s+)?ac\s+to\s+)(\d+)|(\d+)\s*(?:degrees?|°c|celsius))', lower)

        if ac_m:
            target_t = int(ac_m.group(1) or ac_m.group(2) or ac_m.group(3))
            if 16 <= target_t <= 30:
                steps.append(PlanStep(step_id=1, device="AC", capability="AC_SET_TEMPERATURE", parameters={"temperature": target_t}))
                return GeminiStructuredPlan(intent="SET_AC_TEMPERATURE", objective_summary=f"Set AC to {target_t}C", user_request=utterance, steps=steps)
